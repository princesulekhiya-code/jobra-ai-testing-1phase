...
export default function ResumeBuilderPage() {
  ...
  const updateCustomSectionTitle = (sectionIdx: number, title: string) => {
    setCustomSections(customSections.map((section, i) => i === sectionIdx ? { ...section, title } : section));
  };

  const addCustomSectionItem = (sectionIdx: number) => {
    setCustomSections(customSections.map((section, i) => i === sectionIdx ? { ...section, items: [...section.items, ""] } : section));
  };

  const updateCustomSectionItem = (sectionIdx: number, itemIdx: number, value: string) => {
    setCustomSections(customSections.map((section, i) => i === sectionIdx
      ? { ...section, items: section.items.map((item, j) => j === itemIdx ? value : item) }
      : section));
  };

  const removeCustomSectionItem = (sectionIdx: number, itemIdx: number) => {
    setCustomSections(customSections.map((section, i) => i === sectionIdx
      ? { ...section, items: section.items.filter((_, j) => j !== itemIdx) }
      : section));
  };
  ...
  }, [downloading, contact, experience, education, skills, projects, certifications, achievements, languages, customSections, formatting]);
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
          ...
          <div style={{ width: 380, borderRight: "1px solid hsl(var(--border))", overflowY: "auto", padding: 20 }}>
            ...
            <EditorSection icon={<Plus style={{ width: 14, height: 14, color: "hsl(var(--primary))" }} />} title="Custom Sections" onAdd={addCustomSection}>
              {customSections.length === 0 ? (
                <p style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", lineHeight: 1.5 }}>
                  Add any extra section, such as hobbies, volunteering, or interests.
                </p>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {customSections.map((section, sectionIdx) => (
                    <div key={sectionIdx} style={{ border: "1px solid hsl(var(--border))", borderRadius: 8, padding: 10, backgroundColor: "hsl(var(--secondary) / 0.22)" }}>
                      <div style={{ display: "flex", alignItems: "end", gap: 8, marginBottom: 8 }}>
                        <div style={{ flex: 1 }}>
                          <EditorInput label="Section Title" value={section.title} onChange={(v) => updateCustomSectionTitle(sectionIdx, v)} />
                        </div>
                        <button onClick={() => setCustomSections(customSections.filter((_, i) => i !== sectionIdx))} title="Remove section" style={{ background: "none", border: "1px solid hsl(var(--border))", color: "hsl(var(--muted-foreground))", cursor: "pointer", padding: "6px 8px", borderRadius: 6 }}>
                          <X style={{ width: 10, height: 10 }} />
                        </button>
                      </div>
                      {section.items.map((item, itemIdx) => (
                        <div key={itemIdx} style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 6 }}>
                          <input value={item} onChange={(e) => updateCustomSectionItem(sectionIdx, itemIdx, e.target.value)} placeholder="Add detail" style={{ flex: 1, padding: "7px 9px", borderRadius: 5, backgroundColor: "hsl(var(--secondary))", border: "1px solid hsl(var(--border))", color: "hsl(var(--foreground))", fontSize: 11, outline: "none" }} />
                          <button onClick={() => removeCustomSectionItem(sectionIdx, itemIdx)} title="Remove item" style={{ background: "none", border: "1px solid hsl(var(--border))", color: "hsl(var(--muted-foreground))", cursor: "pointer", padding: "6px 8px", borderRadius: 6 }}>
                            <X style={{ width: 10, height: 10 }} />
                          </button>
                        </div>
                      ))}
                      <button onClick={() => addCustomSectionItem(sectionIdx)} style={{ marginTop: 4, width: "100%", padding: "7px 9px", borderRadius: 6, backgroundColor: "hsl(var(--card))", border: "1px dashed hsl(var(--border))", color: "hsl(var(--foreground))", fontSize: 10, cursor: "pointer" }}>
                        + Add Item
                      </button>
                    </div>
                  ))}
